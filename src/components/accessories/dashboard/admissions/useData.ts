...
export const useData = () => {
  ...
  const allAdmissions = useSelector<IState, AdmissionDTO[]>(
    (state) => state.admissions.getAdmissions.data ?? []
  );
  const admissions = useMemo(() => {
    if (onGoing === true) return allAdmissions.filter((e) => e.admitted === 1);
    if (onGoing === false) return allAdmissions.filter((e) => e.admitted === 0);
    return allAdmissions;
  }, [onGoing, allAdmissions]);
