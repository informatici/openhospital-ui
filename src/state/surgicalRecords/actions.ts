...
import { SurgicalRecordControllerApi, Configuration } from "../../generated";
import { applyTokenMiddleware } from "../../libraries/apiUtils/applyTokenMiddleware";
import { IAction } from "../types";
import { GET_SURGICALRECORD_LOADING, GET_SURGICALRECORD_SUCCESS, GET_SURGICALRECORD_FAIL } from "./consts";

const controllerApi = new SurgicalRecordControllerApi(
  new Configuration({ middleware: [applyTokenMiddleware] })
);

const getSurgicalRecordsSuccess = (payload: any): IAction<any, any> => {
  return {
    type: GET_SURGICALRECORD_SUCCESS,
    payload,
  };
};

export const getSurgicalRecords = () => (dispatch: Dispatch<IAction<any, any>>): void => {
  dispatch({ type: GET_SURGICALRECORD_LOADING });

  controllerApi.getSurgicalRecordsUsingGET().subscribe(
    (payload) => {
      if (typeof payload === "object" && !isEmpty(payload)) {
        dispatch(getSurgicalRecordsSuccess(payload));
      } else {
        dispatch({
          type: GET_SURGICALRECORD_FAIL,
